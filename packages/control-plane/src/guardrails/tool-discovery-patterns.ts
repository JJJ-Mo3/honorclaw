...
export const TOOL_DISCOVERY_PATTERNS: RegExp[] = [
  ...
  /(?:list|show|enumerate|print|output)\s+(?:your|all|the|available)\s+(?:tools?|functions?|commands?|capabilities)/i,
  ...
  /(?:show|list|enumerate|describe)\s+(?:all\s+)?(?:your\s+)?(?:available\s+)?(?:tool|function|api|command)\s+(?:names?|list|schema)/i,
];
